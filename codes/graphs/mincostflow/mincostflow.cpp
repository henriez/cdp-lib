struct MinCostFlow {
  ...
  void add_edge(int u, int v, int cap, ll cost){
    adj[u].push_back({v,cap,sz(adj[v]), cost});
    adj[v].push_back({u,0,sz(adj[u])-1, -cost});
  }
  // O(FElog(V))
  lli min_cost_flow(int s, int t, int targetFlow) {
    ...
    vll dist, h(n);
    vi pv, pe;
    spfa(s, h, pv, pe); // only if < 0 costs exist
  ...
  void djk(int s, vll &h, vll &dist, vi &pv, vi &pe) {
    dist.assign(n, INF);
    pv = pe = vi(n,-1);
    dist[s] = 0;
